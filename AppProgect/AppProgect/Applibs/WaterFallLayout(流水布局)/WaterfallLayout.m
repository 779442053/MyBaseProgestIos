...
- (void)prepareLayout {
    ...
    //头部视图
    UICollectionViewLayoutAttributes *layoutHeader = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionHeader withIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
       layoutHeader.frame = CGRectMake(self.sectionInset.left, self.sectionInset.top, self.headerReferenceSize.width, self.headerReferenceSize.height);
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0 || indexPath.row == 1) {
        itemY = itemY + self.headerReferenceSize.height;
    }
    //设置attributes的frame
